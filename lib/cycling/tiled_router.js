...
class TiledRouter {
  ...
  async route(fromLon, fromLat, toLon, toLat) {
    ...
    // タイル v2 (CH 付き) がロードされていれば chQuery、なければ NBA* fallback
    const r = view.hasCh
      ? chQueryOnView(view, fromSnap.id, toSnap.id)
      : nbaStarOnView(view, fromSnap.id, toSnap.id);
